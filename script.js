document.addEventListener('DOMContentLoaded', () => {
    ...
    if (personalCabinetBtn) personalCabinetBtn.addEventListener('click', openPersonalCabinetModal);
    if (balanceAddButton) balanceAddButton.addEventListener('click', openPersonalCabinetModal);
    ...
    const games = [
        { id: 1, name: 'FREE FIRE DIAMOND', image: 'ff.png', description: 'Free Fire üçün diamond paketi. Dərhal hesabınıza yükləyin!', prices: [{ quantity: 100, amount: 2.00 }, { quantity: 210, amount: 4.00 }, { quantity: 340, amount: 6.00 }, { quantity: 560, amount: 9.00 }, { quantity: 680, amount: 11.00 }, { quantity: 900, amount: 14.00 }, { quantity: 1150, amount: 17.00 }, { quantity: 1490, amount: 22.00 }, { quantity: 1720, amount: 26.00 }, { quantity: 2060, amount: 30.00 }, { quantity: 2380, amount: 33.00 }, { quantity: 2950, amount: 41.00 }, { quantity: 'Həftəlik 450', amount: 3.00 }, { quantity: 'Aylıq 2600', amount: 11.00 }], type: 'id_input' },
        { id: 2, name: 'PUBG MOBILE UC', image: 'pubg.jpg', description: 'PUBG Mobile üçün UC (Unknown Cash) alın və oyununuzu təkmilləşdirin.', prices: [{ quantity: 63, amount: 2 }, { quantity: 120, amount: 4 }, { quantity: 240, amount: 7 }, { quantity: 325, amount: 8 }, { quantity: 385, amount: 10 }, { quantity: 445, amount: 12 }, { quantity: 660, amount: 16 }, { quantity: 985, amount: 24 }, { quantity: 1800, amount: 40 }, { quantity: 2785, amount: 65 }, { quantity: 3970, amount: 82 }, { quantity: 8340, amount: 160 }], type: 'id_input' },
        { id: 3, name: 'TIK TOK JETON', image: 'tt.jpg', description: 'TikTok-da canlı yayımlarda hədiyyə göndərmək üçün jetonlar.', prices: [{ quantity: 105, amount: 3 }, { quantity: 215, amount: 5 }, { quantity: 520, amount: 11 }, { quantity: 720, amount: 15 }, { quantity: 1000, amount: 20 }, { quantity: 1500, amount: 30 }], type: 'whatsapp_contact', whatsappNumber: '994777696100' }
    ];
    ...
    function displayGames() {
        if (gameListing) {
            ...
            games.forEach(game => {
                ...
                gameCard.innerHTML = `<a href="game-detail.html?id=${game.id}" style="text-decoration: none; color: inherit;"><img src="${game.image}" alt="${game.name}"><h3>${game.name}</h3></a>`;
    ...
    function updateNotificationCount() {
        ...
        if (notificationCountTop) notificationCountTop.textContent = notifications;
        if (notificationCountBottom) notificationCountBottom.textContent = notifications;
    }
    ...
    // Oyun Detal Səhifəsi Məntiqi (game-detail.html)
    ...
    if (gameDetailContainer) {
        ...
        if (game) {
            ...
            if (game.type === 'id_input') {
                    ...
                    <div class="purchase-options">
                        ...
                        <div class="price-bonus-row">
                            <div class="quantity-control">
                                ...
                                <span class="quantity-display">${game.prices[0].quantity}</span>
                        ...
                        <div class="payment-method-grid">
                            <div class="payment-method-item selected" data-method="balance"><i class="fas fa-wallet method-icon"></i><div class="method-name">Balans</div><div class="method-desc">Hesabınızdakı balans ilə ödəniş</div></div>
                            <div class="payment-method-item" data-method="card"><i class="fas fa-credit-card method-icon"></i><div class="method-name">Kart ilə</div><div class="method-desc">Visa, MasterCard, Maestro</div></div>
                            <div class="payment-method-item" data-method="m10"><i class="fas fa-mobile-alt method-icon"></i><div class="method-name">M10</div><div class="method-desc">Elektron pul kisəsi</div></div>
                        </div>
                ...
                purchaseOptionsHtml = `
                    <div class="purchase-options">
                        ...
                        <a href="https://wa.me/${game.whatsappNumber}?text=${whatsappMessage}" target="_blank" class="whatsapp-button"><i class="fab fa-whatsapp"></i> WhatsApp ilə Əlaqə</a>
                    </div>
                `;
            }
            ...
            if (game.type === 'id_input') {
                ...
                if (orderButton) {
                    orderButton.addEventListener('click', () => {
                        ...
                        const selectedMethodElement = gameDetailContainer.querySelector('.payment-method-item.selected');
                        const selectedMethod = selectedMethodElement ? selectedMethodElement.dataset.method : null;
            ...
            gameDetailContainer.innerHTML = `<h2>Oyun Tapılmadı</h2><p>Axtardığınız oyun tapılmadı. Lütfən, başqa bir oyun seçin.</p>`;
        }
    }
});
